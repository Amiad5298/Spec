...
# Only runtime artifacts (.spec/ for logs/state, *.log files) are ignored
...
def _check_gitignore_has_pattern(content: str, pattern: str) -> bool:
    ...
    Args:
        ...
        pattern: Pattern to search for (e.g., ".spec/", "*.log")
...
def ensure_gitignore_configured(quiet: bool = False) -> bool:
    ...
    Checks for required patterns (.spec/, *.log) and appends them
...
def ensure_agents_installed(quiet: bool = False) -> bool:
    ...
    SPECFLOW artifacts (.spec/ directory, *.log files).
