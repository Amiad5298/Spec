...
**Ticket:** [AMI-50](https://linear.app/amiadingot/issue/AMI-50/phase-14-move-subagent-constants)
**Status:** Draft
**Date:** 2026-02-01
**Labels:** MultiAgent
...
This ticket moves subagent name constants from the Auggie-specific code (`ingot/integrations/auggie.py`) to a neutral, backend-agnostic location (`ingot/workflow/constants.py`). This decoupling is essential for the Pluggable Multi-Agent Support initiative, enabling other backends (Claude Code, Cursor, Aider) to access subagent constants without depending on Auggie-specific code.
...
**Scope:**
- Move 6 existing subagent constants from `ingot/integrations/auggie.py` to new `ingot/workflow/constants.py`
- Create 3 new timeout constants in `ingot/workflow/constants.py` (these don't exist yet in the codebase)
- Update all import statements across the codebase
- Remove constants from `ingot/integrations/auggie.py` entirely
- Expand `ingot/integrations/__init__.py` exports from 4 to 6 subagent constants
...
┌────────────────────────────────────┐
│  ingot/integrations/auggie.py       │
│  ├── INGOT_AGENT_PLANNER        │◄─┬─ ingot/config/settings.py
│  ├── INGOT_AGENT_TASKLIST       │  ├─ ingot/workflow/state.py
│  ├── INGOT_AGENT_TASKLIST_REFINER│  ├─ ingot/integrations/agents.py
│  ├── INGOT_AGENT_IMPLEMENTER    │  ├─ ingot/integrations/__init__.py
│  ├── INGOT_AGENT_REVIEWER       │  └─ tests/test_auggie.py
...
After (target state):
┌────────────────────────────────────┐
│  ingot/workflow/constants.py (NEW)  │
│  ├── INGOT_AGENT_PLANNER        │◄─┬─ ingot/config/settings.py
│  ├── INGOT_AGENT_TASKLIST       │  ├─ ingot/workflow/state.py
│  ├── INGOT_AGENT_TASKLIST_REFINER│  ├─ ingot/integrations/agents.py
│  ├── INGOT_AGENT_IMPLEMENTER    │  ├─ ingot/integrations/__init__.py
│  ├── INGOT_AGENT_REVIEWER       │  ├─ tests/test_auggie.py
│  └── INGOT_AGENT_DOC_UPDATER    │  └─ tests/test_baseline_auggie_behavior.py
└────────────────────────────────────┘
...
From `ingot/integrations/auggie.py` (lines 27-33):

```python
# Subagent names used by INGOT workflow
...
|------|-------------------|--------------|
| `ingot/config/settings.py` | 6 constants (all) | 17-24 |
| `ingot/workflow/state.py` | 6 constants (all) | 12-19 |
| `ingot/integrations/agents.py` | 5 constants (no DOC_UPDATER) | 17-23 |
| `ingot/integrations/__init__.py` | 4 constants (no TASKLIST_REFINER, DOC_UPDATER) | 10-14, 104-107 |
...
These files import from `ingot.integrations.auggie` but only use `AuggieClient`, `AuggieModel`, or other Auggie-specific symbols (not subagent constants):

| File | Imports Used |
|------|--------------|
| `ingot/workflow/runner.py` | `AuggieClient` |
| `ingot/workflow/step1_plan.py` | `AuggieClient` |
| `ingot/workflow/step2_tasklist.py` | `AuggieClient` |
| `ingot/workflow/step3_execute.py` | `AuggieClient`, `AuggieRateLimitError` |
| `ingot/workflow/step4_update_docs.py` | `AuggieClient` |
| `ingot/workflow/review.py` | `AuggieClient` |
| `ingot/workflow/autofix.py` | `AuggieClient` |
| `ingot/workflow/conflict_detection.py` | `AuggieClient` |
| `ingot/cli.py` | `AuggieClient`, `check_auggie_installed`, `install_auggie` |
| `ingot/config/manager.py` | `extract_model_id` |
| `ingot/ui/menus.py` | `list_models` |
| `ingot/integrations/fetchers/auggie_fetcher.py` | `AuggieClient` |
| `ingot/integrations/ticket_service.py` | `AuggieClient` |
| `ingot/integrations/jira.py` | `AuggieClient` |
...
| File | Action | Purpose |
|------|--------|---------|
| `ingot/workflow/constants.py` | **CREATE** | New home for subagent constants (6) + timeout constants (3) |
| `ingot/workflow/__init__.py` | **MODIFY** | Export all 9 constants from new location |
| `ingot/integrations/auggie.py` | **MODIFY** | Remove constant definitions and `__all__` entries |
| `ingot/config/settings.py` | **MODIFY** | Update import source |
| `ingot/workflow/state.py` | **MODIFY** | Update import source |
| `ingot/integrations/agents.py` | **MODIFY** | Update import source |
| `ingot/integrations/__init__.py` | **MODIFY** | Update import source, export all 6 subagent constants |
...
#### Step 1.1: Create ingot/workflow/constants.py
...
#### Step 1.2: Update ingot/workflow/__init__.py

**File:** `ingot/workflow/__init__.py`
...
#### Step 2.1: Update ingot/config/settings.py
...
#### Step 2.2: Update ingot/workflow/state.py
...
#### Step 2.3: Update ingot/integrations/agents.py

**File:** `ingot/integrations/agents.py`
...
#### Step 2.4: Update ingot/integrations/__init__.py
...
> ⚠️ **Scope Expansion:** This migration adds `INGOT_AGENT_TASKLIST_REFINER` and `INGOT_AGENT_DOC_UPDATER` to the public API of `ingot.integrations`. Previously only 4 constants were exported; after this change, all 6 will be exported. Code using `from ingot.integrations import *` will now receive 2 additional symbols. This is intentional for API completeness.

---

### Phase 3: Remove Constants from ingot/integrations/auggie.py

#### Step 3.1: Remove constant definitions from auggie.py

**File:** `ingot/integrations/auggie.py`
...
> **Note:** This file imports only 5 subagent constants (not `INGOT_AGENT_DOC_UPDATER`). This is intentional - the test file only tests the constants it uses. No additional constants should be added.
...
- `test_ingot_agent_constants_are_importable` (lines 59-82)
- `test_subagent_names_match_constants` (lines 498-517)

**Change 1: Update imports in `test_ingot_agent_constants_are_importable` (line 62):**
...
**Note:** The baseline behavior test file tests that constants are importable. After migration, it should test import from the NEW location (`ingot.workflow.constants`), not the old location.
...
Creating `ingot/workflow/constants.py` that is imported by `ingot/integrations/auggie.py` could create circular imports if:
- `ingot/workflow/constants.py` imports from `ingot/integrations/auggie.py` (it doesn't)
...
1. **`ingot/workflow/constants.py`** is a leaf module with NO imports from:
   - `ingot.integrations.*`
   - `ingot.workflow.*`
   - Any project modules
...
2. **Import direction is one-way:**
   ```
   ingot.workflow.constants  (no project imports)
         ↑
   ingot.config.settings, ingot.workflow.state, ingot.integrations.agents, etc.
   ```

3. **Verification:** After implementation, run:
   ```bash
   python -c "from ingot.workflow.constants import INGOT_AGENT_PLANNER; print('OK')"
   python -c "from ingot.config.settings import Settings; print('OK')"
   ```
...
python -c "from ingot.workflow.constants import INGOT_AGENT_PLANNER, INGOT_AGENT_TASKLIST, INGOT_AGENT_TASKLIST_REFINER, INGOT_AGENT_IMPLEMENTER, INGOT_AGENT_REVIEWER, INGOT_AGENT_DOC_UPDATER; print('Subagent constants import OK')"

# Test direct import of timeout constants from new location
python -c "from ingot.workflow.constants import DEFAULT_EXECUTION_TIMEOUT, FIRST_RUN_TIMEOUT, ONBOARDING_SMOKE_TEST_TIMEOUT; print('Timeout constants import OK')"

# Test package-level import from ingot.workflow
python -c "from ingot.workflow import INGOT_AGENT_PLANNER, DEFAULT_EXECUTION_TIMEOUT; print('Package import OK')"
```
...
python -c "from ingot.integrations.auggie import INGOT_AGENT_PLANNER" 2>&1 | grep -q "ImportError" && echo "Correctly removed from auggie.py"
```
...
assert INGOT_AGENT_DOC_UPDATER == 'ingot-doc-updater'
...
grep -rn "from ingot.integrations.auggie import" --include="*.py" . | grep "INGOT_AGENT" | grep -v "auggie.py" | grep -v "__pycache__"
# Expected: 0 matches
...
**Mitigation:**
- `ingot/workflow/constants.py` has NO project imports
...
- [ ] `ingot/workflow/constants.py` created with all 6 subagent constants + 3 timeout constants
- [ ] `ingot/workflow/__init__.py` exports the new constants (9 total)
- [ ] `ingot/config/settings.py` imports from new location
- [ ] `ingot/workflow/state.py` imports from new location
- [ ] `ingot/integrations/agents.py` imports from new location
- [ ] `ingot/integrations/__init__.py` imports from new location (all 6 subagent constants)
- [ ] `ingot/integrations/auggie.py` constants and `__all__` entries removed
...
- [ ] Direct imports from `ingot.workflow.constants` work (subagent + timeout constants)
- [ ] Package-level imports from `ingot.workflow` work
- [ ] Constants no longer importable from `ingot.integrations.auggie`
...
- [ ] No imports of INGOT_AGENT_* from `auggie.py` remain
...
|------|-------|-------------|
| `ingot/integrations/auggie.py` | 27-33 | Current constant definitions (including comment) |
| `ingot/integrations/auggie.py` | 755-761 | Current `__all__` exports (including comment) |
| `ingot/config/settings.py` | 17-24, 81-86 | Import and usage |
| `ingot/workflow/state.py` | 12-19, 146-151 | Import and usage |
| `ingot/integrations/agents.py` | 17-23, 187-217 | Import and usage (5 constants, no DOC_UPDATER) |
| `ingot/integrations/__init__.py` | 10-14, 104-107 | Re-export (4 constants currently) |
...
### ingot/workflow/constants.py (Complete)
...
| File | Change Type |
|------|-------------|
| `ingot/workflow/constants.py` | CREATE - new file with 6 moved subagent constants + 3 NEW timeout constants |
| `ingot/workflow/__init__.py` | ADD - import and export all 9 constants |
| `ingot/config/settings.py` | CHANGE - import 6 subagent constants from `ingot.workflow.constants` |
| `ingot/workflow/state.py` | CHANGE - import 6 subagent constants from `ingot.workflow.constants` |
| `ingot/integrations/agents.py` | CHANGE - import 5 subagent constants from `ingot.workflow.constants` |
| `ingot/integrations/__init__.py` | CHANGE - import 6 subagent constants from `ingot.workflow.constants` (was 4) |
| `ingot/integrations/auggie.py` | REMOVE - delete 6 constant definitions and `__all__` entries |
| `tests/test_auggie.py` | CHANGE - import 5 subagent constants from `ingot.workflow.constants` |
| `tests/test_baseline_auggie_behavior.py` | CHANGE - import 6 subagent constants from `ingot.workflow.constants` + fix docstring |
